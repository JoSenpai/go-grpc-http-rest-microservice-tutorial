...
var (
	...
	// onceInit gurantee initialize logger only once
	onceInit sync.Once
)

// customTineEncoder encode Time to our custom format
// This example how we can custmize zap default functionality
func customTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format(customTimeFormat))
}

// Init initializes log by input parameters
// lvl - global log level: Debug(-1), Info(0), Warn(1), Error(2), DPanic(3)
...
func Init(lvl int, timeFormat string) error {
	...
	onceInit.Do(func() {
		...
		// Kubernetes interpretes os.Stdout log items as INFO and os.Stderr log items
		// as ERROR by default.
		...
		// From a zapcore.Core, its easy to construct a logger.
		Log = zap.New(core)
